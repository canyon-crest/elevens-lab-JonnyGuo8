	...
	private boolean playJQKIfPossible() {
		...
		if(findJQK(cIndexes)) {
			...
			for (Integer kObj : cIndexes) {
				int a = kObj.intValue();
				if (cardAt(a).rank().equals("jack")) {
					foundJack = true;
					j=a;
				} else if (cardAt(a).rank().equals("queen")) {
					foundQueen = true;
					q=a;
				} else if (cardAt(a).rank().equals("king")) {
					foundKing = true;
					k=a;
				}
			}
